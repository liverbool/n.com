...
class Writer
{
	...
	{
		$id = $this->genInfo['imdb_id'] ? $this->genInfo['imdb_id'] : $this->genInfo['tmdb_id'];
	...
	private function addJumboBackground()
	{
		if ($this->provider->name == 'imdb' || $this->options->saveTmdbImages())
		{
			...
			if ($img)
			{
				...
				$this->images->saveSingle($img, $this->genInfo['imdb_id'], 'imdb/bgs/');

				$this->genInfo['background'] = "imdb/bgs/{$this->genInfo['imdb_id']}.jpg";
			}		
		}
		else
		{
