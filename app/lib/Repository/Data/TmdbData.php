...
class TmdbData implements DataRepositoryInterface, SearchProviderInterface
{
	...
	{
		return (isset($this->raw['original_title']) ? $this->raw['original_title'] : $this->raw['original_name']);
	}
	...
	public function getFullAllSeasons(Title $series)
	{
		...
		return $this->compileFullAllSeasons(json_decode($response, true), $series->season);
	}
	...
	private function compileFullAllSeasons(array $data, $seasons)
	{
		...
		foreach ($seasons as $k => $v)
		{
			if ( isset($data['season/' . $v->number]) )
			{
				...
				$compiled[$k] = array('title' => $title, 'overview'  => $overview,
	...
	public function getImages()
	{
		if ( ! $this->images)
		{
			$this->images = $this->images($this->raw['images']['backdrops']);
		}
