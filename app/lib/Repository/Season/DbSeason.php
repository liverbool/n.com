...
class DbSeason implements SeasonRepositoryInterface
{
    ...
    public function prepareSingle(Title $title, $num)
    {
        ...
        if ( $this->provider->name == 'tmdb' && $title->tmdb_id)
        {
            ...
            if ($first->fully_scraped && $first->title_tmdb_id)
            {
                return $title;
            }
    ...
    public function handleSingleLoading($title, $num)
    {
        ...
        if ($season->fully_scraped || $this->provider->name == 'db' || ! $title->allow_update)
        {
            return $title;
        }
        ...
        $this->dbWriter->CompileInsert('seasons', array('id' => $season->id, 'fully_scraped' => 1))->save();
